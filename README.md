...
The repository examples (`data/raw/open-dev/input.txt`, `data/raw/open-dev/pred.txt`, `src/predict.sh`) define the **code execution / verification format** for this repo.
...
This repository now implements a language-aware character n-gram system. The source is organized as:

* `src/main.py`: canonical train/test entrypoint.
* `src/data/training_data.py`: corpus loading and normalization.
* `src/models/char_ngram_model.py`: core interpolated character n-gram model.
* `src/models/language_aware_lm.py`: per-language model orchestration + checkpointing.
* `src/identification/language_identifier.py`: script-aware language ID.
* `src/config/constants.py`: shared language/constants definitions.
...
Train the model and save the checkpoint in `work`:

```bash
PYTHONPATH=src python3 -m main train --work_dir work
```

Generate predictions for `data/raw/open-dev/input.txt` and save to `pred.txt`:

```bash
PYTHONPATH=src python3 -m main test --work_dir work --test_data data/raw/open-dev/input.txt --test_output pred.txt
```
...
```bash
python3 grader/grade.py output/pred.txt data/raw/open-dev/answer.txt --verbose
```
...
You should be able to emulate the grading using the example data by running `bash grader/grade.sh data/raw/open-dev`, which will write your results to the `output` directory.
